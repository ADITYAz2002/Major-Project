...
for t = 1:round(total_time / time_step)
    ...
    if second_car_started
        if signal_color == "red" && abs(second_car_position_x - signal_position) <= range && ~second_car_stopped_message_displayed
        ...
        if second_car_speed_x > 0
            second_car_position_x = min(second_car_position_x + second_car_speed_x * time_step, road_length);
        end
        
        % Move the second car in y-axis if it has stopped in x-axis
        if second_car_speed_x == 0
            second_car_position_y = min(second_car_position_y + second_car_speed_y * time_step, signal_position);
        end
    end
    ...
    if second_car_started
        plot(second_car_position_x, second_car_position_y, 'o', 'MarkerSize', 10, 'MarkerFaceColor', second_car_color); % Second Car
    end
    
    % Add green arrow annotation when the signal turns red
    if strcmp(signal_color, 'red') && ~second_car_stopped_message_displayed
        annotation('arrow', [0.5, 0.5], [0.2,0.27], 'Color', 'green', 'LineWidth', 2, 'HeadWidth', 15, 'HeadLength', 15);
    end
