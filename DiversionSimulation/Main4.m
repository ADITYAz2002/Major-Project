...
second_car_speed = 10;    % Speed of the second car
...
for t = 1:round(total_time / time_step)
    ...
    if second_car_started
        second_car_position = min(second_car_position + second_car_speed * time_step, road_length);
        
        % Check if the second car should stop at the signal
        if second_car_position >= signal_position - range && second_car_position < signal_position && ~second_car_stopped_message_displayed
            disp('Second car stops at the signal.');
            second_car_stopped_message_displayed = true; % Set the flag to true
            second_car_speed = 0; % Second car stops
        end
    end
    ...
    if second_car_started
        plot(second_car_position, 0, 'o', 'MarkerSize', 10, 'MarkerFaceColor', second_car_color); % Second Car
    end
