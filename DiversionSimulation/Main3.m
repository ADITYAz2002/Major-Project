...
car_speed = 8;             % Car speed (in meters per second)
obstruction_position = 50;  % Position of the obstruction
total_time = road_length/car_speed;            % Total simulation time (in seconds)
...
% Flag to check if obstruction is detected
...
for t = 1:round(total_time / time_step)
    ...
    if abs(car_position - obstruction_position) <= 5 && ~obstruction_detected
    ...
    elseif obstruction_detected
        ...
        disp(['Signal color: ' signal_color]); % Display signal color when obstruction is not detected
    end
